...
function renderAuth(){
  ...
  if(currentUser){
    authArea.innerHTML=`
      ...
      <p>Ранг: <b class="rank-${currentUser.rank}">${currentUser.rank}</b></p>
...
function renderRegister(){
  ...
  document.getElementById("reg-btn").addEventListener("click",()=>{
    ...
    if(!/^[A-Za-z]+$/.test(nick)){showModal("Ник только на английском!"); return;}
    if(accounts.some(a=>a.username.toLowerCase()===nick.toLowerCase())){showModal("Ник уже занят!"); return;}
...
function renderAdmin(){
  ...
  let html=`<input id="search-input" placeholder="Поиск по нику или ID"/>
